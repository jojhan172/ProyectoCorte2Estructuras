...
public class Main {
    ...
    * */
    ...
    // esta función verifica si el numero ingresado por el usuario es valido para nuestro menu
    ...
    private Object[][] Ordenar(){ // Uso del algoritmo de Bubble sorting -> compara por parejas // O(n^2)
        Object[] aux;
        for (int i = 0; i < this.data.length; i++){ // aquí usamos el operador de casting -> (int) -> para poder comprar nuestros datos dabido a que estamos usando una matriz tipo Object[][]
    ...
    private void modificar(int target, int columna){
        ...
        if (index == -1){ // si no existe simplemente avisamos
            ...
            switch (columna) {
                case 1:
                    System.out.println("Ingresa el nuevo codigo del producto");
            ...
            System.out.printf("%-12s%-22s%-12s%-15s%-15s%n", "1. Codigo", "2. Nombre", "3. Cantidad","4. Valor IVA", "5. Precio Unitario");
            System.out.printf("%-12d%-22s%-12d%-15s%-15.1f%n", this.data[index][0], this.data[index][1], this.data[index][2],this.data[index][3], this.data[index][4]);
        }
    }
    ...
    // metodo para imprimir la tabla completa, sin los totales calculados (Solo los datos que el usuario ingreso) 
            ...
            switch (opcion) {
                ...
                case 4: 
                    ...
                    if (index == -1){
                        ...
                        System.out.printf("%-12s%-22s%-12s%-15s%-15s%n", "1. Codigo", "2. Nombre", "3. Cantidad","4. Valor IVA", "5. Precio Unitario");
                        System.out.printf("%-12d%-22s%-12d%-15s%-15.1f%n", main.data[index][0], main.data[index][1], main.data[index][2],main.data[index][3], main.data[index][4]);
                    }
                    opcion = menu();
                    break;
                ...
                default:
                    System.out.println("Algo salio mal, intenta crear una nueva factura");
                    opcion = menu();
                    break;
            }
        }
